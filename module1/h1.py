...
def detect(gray_img, frame):
    ...
    for (x, y, w, h) in faces:
        ...
        eyes = eye_cascade.detectMultiScale(roi_grayimg, 1.1, 22)
        for (ex, ey, ew, eh) in eyes:
            cv2.rectangle(frame, (ex + x, ey + y), (ex + x + ew, ey + y + eh),
                          (0, 255, 0), 2)
        smiles = smile_cascade.detectMultiScale(roi_grayimg, 1.7, 22)
        for (sx, sy, sw, sh) in smiles:
            cv2.rectangle(frame, (sx + x, sy + y), (sx + x + sw, sy + y + sh),
                          (0, 0, 255), 2)
        # for (sx, sy, sw, sh) in smiles:
